...
Markdown Viewer의 차별점은 로컬 우선 다중 문서 관리, 리뷰 워크플로, 풍부한 시각적 렌더러, 다양한 내보내기, 선택적 링크 공유를 계정이 필요 없는 하나의 인터페이스에 결합했다는 점입니다. 같은 프로젝트를 호스팅 웹 앱, 설치형 PWA, 자체 호스팅 Docker 배포 또는 Neutralino 데스크톱 애플리케이션으로 사용할 수 있으며, 네트워크에 의존하는 동작은 문서에 명확히 표시됩니다.

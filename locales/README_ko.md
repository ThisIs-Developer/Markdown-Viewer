...
- **가져오기 및 내보내기:** 로컬 파일뿐 아니라 브랜치, 태그 또는 커밋 SHA를 지정한 공개/비공개 GitHub 콘텐츠를 가져올 수 있습니다. 발견된 모든 Markdown 파일은 새 탭을 열지 않고 Explorer에 저장됩니다. Markdown, 독립 실행형 HTML, PNG, 브라우저 인쇄/PDF 저장, 레거시 래스터 PDF로 내보낼 수 있습니다.
...
- GitHub 가져오기는 선택한 저장소/폴더에서 발견된 모든 Markdown 파일을 표시합니다.
- 로컬 GitHub 자격 증명 보관소에는 이름이 지정된 PAT를 최대 50개 저장할 수 있습니다. 각 토큰 이름은 60자로 제한됩니다.
...
- 일반 웹 문서는 문서별 IndexedDB 레코드에 저장되고, 데스크톱 문서는 고정된 `Documents/Markdown Viewer Vault`에 저장됩니다.
- 프라이빗 모드를 켜면 해당 세션의 새 문서 상태 저장이 중지됩니다. 기존 일반 문서와 Secret Workspace 데이터는 삭제되지 않습니다.
- **작업공간 재설정**은 문서, 폴더, 설정, 리뷰 데이터, Secret Workspace 데이터, 기록 및 휴지통을 영구적으로 삭제합니다.
- **저장 및 백업**에서 폴더 구조를 유지하는 ZIP을 만들거나 가져올 수 있습니다. 백업을 가져오면 확인 후 현재 작업공간이 완전히 대체됩니다.
- 비공개 저장소용 PAT는 로컬 AES-GCM 암호화 보관소에 저장되며 작업공간 백업에는 포함되지 않습니다. 이 보관소는 운영 체제 키체인이 아닙니다. PAT는 `api.github.com`에만 전송되며 로컬에서 삭제해도 GitHub에서 폐기되지는 않습니다.
...
> **백업:** **작업공간 재설정**은 모든 로컬 작업공간 데이터를 영구적으로 삭제합니다. 보관할 데이터가 있다면 먼저 **저장 및 백업**에서 ZIP 백업을 만드세요.

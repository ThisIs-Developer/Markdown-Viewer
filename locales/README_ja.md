...
Markdown Viewer の特長は、ローカルファーストの複数文書管理に、レビューワークフロー、豊富なビジュアルレンダラー、多様なエクスポート、任意のリンク共有を組み合わせ、アカウント不要のひとつのインターフェースで提供する点です。同じプロジェクトをホスト型 Web アプリ、インストール可能な PWA、セルフホスト型 Docker デプロイ、Neutralino デスクトップアプリケーションとして利用でき、ネットワークに依存する動作はドキュメントで明示されています。

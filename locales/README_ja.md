...
Markdown Viewer は、開発者、ライター、学生、研究者など、`.md`／`.markdown` ファイルを扱うすべての人に向けたオープンソースのローカルファーストワークスペースです。単なるプレーンテキストにとどまりません。複数の文書を整理し、Markdown を変更せずに選択箇所に紐づくコメントでレビューし、図表、地図、チャート、数式、3D モデル、音楽をひとつの集中できるワークスペースで描画できます。

文書を Share Snapshot リンクにしてすばやく共有したり、アクセス制御付きの Live Share でリアルタイム共同編集、ライブカーソル、選択箇所に紐づくコメント、スレッド形式の返信を利用したりできます。通常の編集は端末上で行われます。アカウントは不要で、アプリケーションには広告、分析、テレメトリ、サブスクリプションがありません。
...
- **編集とレビュー：** エディター、分割表示、プレビューを切り替えられます。書式設定ツール、生産性向上のショートカット、独自の元に戻す／やり直す、検索と置換、LTR／RTL、選択箇所に紐づくコメント、スレッド形式の返信を利用できます。

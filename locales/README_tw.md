...
Markdown Viewer 是一款適用於 `.md` 與 `.markdown` 文件的開放原始碼編輯器。它將純文字編輯器、已清理的預覽、多文件工作區、審閱工具、豐富的轉譯器、導入／匯出、Share Snapshot 與 Live Share 整合在單一應用程式中。

Markdown Viewer 的獨特之處在於，它將本機優先的多文件管理、審閱工作流程、豐富的視覺化轉譯器、靈活的匯出方式及選用的連結分享整合在一個無需帳號的介面中。同一專案既可作為託管 Web 應用程式與可安裝的 PWA，也能透過 Docker 自行託管或作為 Neutralino 桌面應用程式使用；文件會明確說明依賴網路的行為。

本工具適合想在不建立帳號的情況下使用 Markdown 的開發人員、技術寫作者、學生及研究人員。一般編輯、預覽轉譯、本機檔案導入、文件整理及大多數匯出操作都在裝置上完成。需要網路的功能會在本文件中明確說明。

...
Markdown Viewer 的独特之处在于，它将本地优先的多文档管理、审阅工作流、丰富的可视化渲染器、灵活的导出方式和可选的链接分享整合到一个无需账号的界面中。同一项目既可用作托管 Web 应用和可安装的 PWA，也可通过 Docker 自托管或作为 Neutralino 桌面应用使用；文档会明确说明依赖网络的行为。

...
- Private repository access accepts fine-grained and classic PATs automatically. Multiple named tokens can be added, selected, and removed at any time during the current session; tokens are held only in memory and disappear on reload or app close. GitHub validates each token when added, and the selector shows GitHub's reported expiry when that response header is available.

...
| GitHub import | Repository/path/ref request and selected content downloads; optional named PATs for private repositories | Public content: GitHub API and `raw.githubusercontent.com`; private content and PAT: `api.github.com` only | Fine-grained and classic PATs are accepted automatically, remain in memory for the current session, support optional earlier in-session expiry, and are never written to application storage |

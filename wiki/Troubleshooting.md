...
- Select the intended named token before importing. If GitHub rejects it or reports that it cannot access the repository, replace the token or update its repository permissions. Check the inline access status for the specific failure. Tokens are session-only and must be added again after reload or app close; you can remove a named token at any time before then.

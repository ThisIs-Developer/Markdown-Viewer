...
- Select the intended named token before importing. If GitHub rejects it or reports that it cannot access the repository, check the GitHub-branded error toast, then replace the token or update its repository permissions. Tokens persist locally across refreshes and app restarts until you remove them or clear the site's/app's local data. When GitHub or the browser does not expose a readable expiration date, the badge says **Expiry unknown**; this does not mean the token is permanent.

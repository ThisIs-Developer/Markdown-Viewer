...
- Select the intended named token before importing. If GitHub rejects it or reports that it cannot access the repository, check the GitHub-branded error toast, then replace the token or update its repository permissions. Tokens are session-only and must be added again after reload or app close; you can remove a named token at any time before then. **Expiry unavailable** means GitHub or the browser did not expose an expiration header, not that the token is permanent.

...
- **Document Outline** opens from the square-menu button in the toolbar sequence Emoji, Find & Replace, Full Screen, Document Outline. Its right sidebar displays an H1–H6 heading tree with collapsible branches and hierarchy guides. The shared Find & Replace header includes Collapse/Expand All beside Close, using the File Sidebar's folding icons and interaction pattern. Both panels have text-only header titles, a default width of 340px, and the File Sidebar's horizontal resize handles, keyboard controls, and reset behavior. It updates as you edit or switch documents and navigates within Editor, Preview, and Split without changing the selected mode. Code blocks are excluded; documents without headings show an empty state. The outline supports keyboard navigation, adapts to small screens, and is available in read-only documents. It is generated locally, uses no separate persistent storage, and stays out of exports.

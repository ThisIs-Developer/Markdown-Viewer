...
GitHub access tokens are session-only. They remain in memory and are never written to application storage, so they disappear on reload or app close and do not require a protection passphrase or unlock step. You can remove any named token at any time. Adding, removing, or rejecting a token is reported in the inline access status inside the private-access panel. Local removal does not revoke a PAT on GitHub, so revoke it separately in GitHub settings if it may have been exposed.

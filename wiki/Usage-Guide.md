...
Direct Markdown file URLs import immediately into Explorer. Repository and folder URLs resolve the default branch, an explicit branch (including names containing `/`), a tag, or a commit to an immutable commit SHA before listing files. The searchable, collapsible tree contains every Markdown file found and uses the same compact type scale as other application modals. Repository name and metadata stay on one line: explicit tree/blob URLs show the branch or ref beside the short commit, while a repository URL shows only the short commit. The toolbar places the selected-file count in a compact badge beside matching borderless controls for select/deselect all and collapse/expand all folders. Default-branch imports use a repository-named Explorer folder. Nested GitHub directory paths are preserved, and imported files remain closed until you select one in Explorer.
...
GitHub access tokens are session-only. They remain in memory and are never written to application storage, so they disappear on reload or app close and do not require a protection passphrase or unlock step. You can optionally choose an earlier expiry date; the app removes the token from memory at that time. Adding, removing, expiring, or rejecting a token is reported through the application toast surface. Local removal does not revoke a PAT on GitHub, so revoke it separately in GitHub settings if it may have been exposed.

...
Public repositories need no credentials. For a private repository, expand **Private repository access**. Choose a fine-grained PAT (recommended) with **Contents: Read-only** for multiple selected repositories or all repositories under one owner. Choose a classic PAT only when compatibility across resource owners or another fine-grained limitation requires it; the classic `repo` scope grants broader access. The importer first tries public access and sends the token only to `api.github.com` when authenticated access is needed. Private file content is downloaded through GitHub's Contents API rather than the raw-content host.

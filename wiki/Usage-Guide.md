...
Public repositories need no credentials. For a private repository, expand **Private repository access**, choose fine-grained (recommended) or classic, enter a name and token, choose a disposal time, and select **Add access**. After adding, the form closes and a dropdown lets you select among multiple named tokens before choosing **Import**. Use **Add another**, **Unlock**, or **Remove** to manage entries. The importer first tries public access and sends the selected token only to `api.github.com` when authenticated access is needed. Private file content is downloaded through GitHub's Contents API rather than the raw-content host.

**End of this session** never writes the PAT to application storage. The optional 1-, 7-, or 30-day disposal choices require a separate 12-character protection passphrase and store only PBKDF2-derived, AES-GCM authenticated ciphertext. The passphrase is never stored. Expired entries become unusable and are removed while the app is running or the next time it starts. Local removal does not revoke a PAT on GitHub, so revoke it separately in GitHub settings if it may have been exposed.

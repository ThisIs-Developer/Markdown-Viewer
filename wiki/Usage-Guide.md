...
Direct Markdown file URLs import immediately into Explorer. Repository and folder URLs resolve the default branch, an explicit branch (including names containing `/`), a tag, or a commit to an immutable commit SHA before listing files. The searchable, collapsible tree contains every Markdown file found, shows the repository with its branch and short commit side by side, and uses the same compact type scale as other application modals. The toolbar shows the selected-file count and provides matching icon controls for select/deselect all and collapse/expand all folders. Default-branch imports use a repository-named Explorer folder. Nested GitHub directory paths are preserved, and imported files remain closed until you select one in Explorer.
...
GitHub access tokens are session-only. They remain in memory and are never written to application storage, so they disappear on reload or app close and do not require a protection passphrase or unlock step. Local removal does not revoke a PAT on GitHub, so revoke it separately in GitHub settings if it may have been exposed.

...
- **Live Share routing:** Restored production Live Share WebSocket upgrades by routing `/live-room/*` through Cloudflare Pages Functions to the Durable Object relay. Added a build regression check for the WebSocket route and removed stale local-asset preload hints that generated browser warnings without improving resource use.
